...
function defaultChromePath() {
  // Prefer common desktop Chrome/Chromium locations so local experiments work
...
function parseArgs(argv) {
  ...
  // entrypoints can share the exact same crawler configuration.
  const args = {
    url: "",
    input: "",
    limit: 15,
...
function readTargetsFromInput(csvPath, offset, limit) {
  ...
  // is carried through to output so experiments can be joined back to source data.
...
function readTargets(args) {
  ...
  // historical behavior of the original single-file crawler.
  if (args.url) {
