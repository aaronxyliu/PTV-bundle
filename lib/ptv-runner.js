...
async function classifyPageState(page) {
  // These diagnostics help interpret missing detections without trying to
  // bypass logins, bot checks, or access controls.
...
async function handleConsent(page, policy) {
  ...
  // Prefer rejection/essential-only choices in the default policy so the crawl
  // can proceed while keeping the page interaction privacy-preserving.
  const labels =
...
async function simpleScroll(page) {
  // Some libraries initialize lazily after viewport movement. A short scroll is
  // enough to trigger common lazy paths without turning the crawl into browsing.
...
async function waitForPtvResult(page, timeoutMs) {
  // PTV writes its detection result into meta tags on the page. Waiting on the
  // content attribute keeps this runner independent from PTV internals.
...
async function forcePtvDetect(page) {
  await page.evaluate(() => {
    ...
    // This fallback helps when the first auto-detect fires before late bundle
...
async function collectPtv(page, timeoutMs) {
  try {
    ...
    // Give PTV one explicit second chance before treating the page as a timeout.
...
async function crawlVariant(browser, target, args, instrument) {
  // A variant is one visit to the target URL. The paired experiment calls this
  // twice: once as baseline, once with response instrumentation enabled.
  ...
  try {
    // Install interception before navigation so first-party and early chunk
    // scripts can be transformed before execution.
