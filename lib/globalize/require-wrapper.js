...
function patchRequireFunction(code, candidate, options = {}) {
  ...
  const captureLocation = {
    ...
    require_name: candidate.requireName,
    module_id_parameter: moduleIdName,
  };
  const capture = captureStatement(moduleIdName, nodeCode(code, executedExports), captureLocation);
  ...
  if (
    ...
    if (returnStatement && returnStatement.argument === parent) {
      ...
      const sequenceCapture = captureStatement(
        moduleIdName,
        nodeCode(code, sequenceExports),
        captureLocation,
      );
