...
function patchEntryDeclarationFallback(ast, code, options = {}) {
  ...
  for (const item of candidate.rawRequires) {
    ...
    lines.push("window.varStorage.entryLocations = window.varStorage.entryLocations || {};");
    lines.push(
      `window.varStorage.entryLocations[${JSON.stringify(item.localName)}] = ${JSON.stringify({
        bundler: "webpack",
        webpack_pattern: "entry-require-globalization",
        script_url: options.scriptUrl || "",
        capture: "entry-require",
        require_name: item.requireName,
        local_name: item.localName,
        module_id: String(item.moduleId),
      })};`,
    );
    exposed.push(item);
  }
